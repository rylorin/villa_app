...
class PostController extends Controller
{
    ...
    public function renderArchive(array $criteria = array(), array $parameters = array(), Request $request = null)
    {
        ...
        $parameters = array_merge(array(
            ...
            'template' => 'RylReignThemeBundle:Post:archive.%s.twig',
        ), $parameters);

        $response = $this->render(sprintf($parameters['template'], $request->getRequestFormat()), $parameters);
...
/*        	this->get('sonata.classification.manager.category')->findOneBy(array(
        	    'slug'    => $path[0],
        	    'enabled' => true,
        	));*/
        	...
        	$parameters = array(
        	    'category' => $category,
        	    'mediasrc' => $provider->generatePublicUrl($media, $format),
        	    'template' => 'RylReignThemeBundle:Post:category.%s.twig',
        	);

        	return $this->renderArchive($criteria, $parameters, $request);
     }
